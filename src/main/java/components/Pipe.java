...
public class Pipe extends Component{
    ...
    public void beginCollision(GameObject collidingObject, Contact contact, Vector2f contactNormal){
        ...
        if (playerController != null){//player colliding
            switch (direction){
                case Up:
                    if(contactNormal.y < entranceVectorTolerance){return;}
                    break;
                case Right:
                    if (contactNormal.x < entranceVectorTolerance){return;}
                    break;
                case Down:
                    if (contactNormal.y > -entranceVectorTolerance){return;}
                    break;
