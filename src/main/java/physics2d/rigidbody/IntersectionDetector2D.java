...
public class IntersectionDetector2D {
    ...
    public static boolean pointOnLine(Vector2f point, Line2D line){
        ...
        if(dy == 0f){
            return JMath.compare(point.y, line.getStart().y);
        }
        float k = dy/dx;

        float b = line.getEnd().y - (k*line.getEnd().x);
    ...
    public static boolean lineAndAABB(Line2D line, AABB box){
        ...
        unitVector.y = (unitVector.y != 0) ? 1.0f /unitVector.x : 0f;
    ...
    public static boolean raycast(AABB box, Ray2D ray, RaycastResult result){
        ...
        if(result != null){
            ...
            Vector2f normal = new Vector2f(ray.getDirection()).sub(point);
            normal.normalize();
    ...
    public static boolean raycast(Box2D box, Ray2D ray, RaycastResult result){
        ...
        for(int i = 0 ; i < 2; i++){
            if(JMath.compare(f.get(i),0)){
                ...
                f.setComponent(i, 0.00001f);//avoid /0
            }
    ...
    public static boolean AABBAndBOX2D(AABB b1, Box2D b2){
        ...
        JMath.rotate(axisToTest[2],b2.getRigidbody().getRotation(),new Vector2f());
        JMath.rotate(axisToTest[3],b2.getRigidbody().getRotation(),new Vector2f());
